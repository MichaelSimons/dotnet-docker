...
namespace Dotnet.Docker
{
    ...
    public class DockerfileShaUpdater : FileRegexUpdater
    {
        ...
        private Dictionary<string, string> urls = new Dictionary<string, string>{
            ...
            {"runtime", "https://dotnetcli.azureedge.net/dotnet/Runtime/$VERSION/dotnet-runtime-$VERSION-$OS-$ARCH.$EXT"},
            {"aspnet", "https://dotnetcli.azureedge.net/dotnet/aspnetcore/Runtime/$VERSION/aspnetcore-runtime-$VERSION-$OS-$ARCH.$EXT"},
            {"sdk", "https://dotnetcli.azureedge.net/dotnet/Sdk/$VERSION/dotnet-sdk-$VERSION-$OS-$ARCH.$EXT"},
            ...
            string product, string dockerfileVersion, string repoRoot, Options options)
        {
            ...
            return shaVariableRegex.Matches(versions)
                .Select(m => 
                {
                    string variable = m.Groups["shaVariable"].Value;
                    string[] parts = variable.Split('|');
                    string arch = parts.Length >= 5 ? parts[3] : string.Empty;
                    string os = parts.Length >= 4 ? parts[2] : string.Empty;
                    string archPattern = arch == string.Empty ? string.Empty : arch +"\\|";
                    string osPattern = os == string.Empty ? string.Empty : os +"\\|";
                    string pattern = $"\"(?<shaVariable>{Regex.Escape(product)}\\|{Regex.Escape(dockerfileVersion)}\\|{osPattern}{archPattern}sha)\": \"(?<shaValue>.*)\"";
                    Trace.TraceInformation($"Updating {variable} - {pattern}");
                    string productVersionLookup = product == "lzma" ? "sdk" : product;
                    return new DockerfileShaUpdater()
                    {
                        _productName = product,
                        // TODO handle lzma
                        _productVersion = ProductVersionUpdater.GetProductVersion(productVersionLookup, dockerfileVersion, versions),
            ...
            IEnumerable<IDependencyInfo> dependencyBuildInfos, out IEnumerable<IDependencyInfo> usedBuildInfos)
        {
            ...
            string downloadUrl = urls[_productName]
                .Replace("$VERSION", _productVersion)
                .Replace("$OS", _os)
                .Replace("$ARCH", _arch)
                .Replace("$EXT", _os.Contains("win") ? "zip": "tar.gz")
                .Replace("..", "."); // log issue on PWSH for this
        ...
        {
            string productVersion = _productVersion;
            ...
            {
                productVersion = ProductVersionUpdater.GetProductVersion("runtime", _dockerfileVersion, _versions);
            }
